...
public class SearchFiles {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      // SpanTermQuery stq = new SpanTermQuery(new Term(field, line));
      // SpanPositionRangeQuery query = new SpanPositionRangeQuery(stq, 1, 10);
      // Query query = new PositionQuery(new Term(field, line));
      // PhraseQuery query = new PhraseQuery();
      // query.add(new Term(field, "abc"));
      // query.add(new Term(field, "def"));
      // Analyzer analyzer = new StandardAnalyzer(Version.LUCENE_31);
      // QueryParser parser = new QueryParser(Version.LUCENE_31, field,
      // analyzer);
      // Query query = parser.parse("GATCCTCC ATATACAA");
      // Query query = new TermQuery(new Term(field, "ATATACAA".toLowerCase()));
